...
p = Database("event_preprocessing")
@p.register
@Data.from_class()
class TrialsComputer:
    ...
    def location(folder, session, block):
        return Path(folder) / session / "Blocks" / f"Block_{block:02d}" / "trial_indexed_events.tsv"

    @staticmethod
    @cache(lambda f, a: a.to_csv(f, sep="\t"))
    def compute(db: DatabaseInstance, out_location, selection): 
        events = db.run_action("load", "raw_events", selection, single=True)
    ...
    @staticmethod
    def load(db: DatabaseInstance, out_location, selection): return pd.read_csv(out_location, sep="\t")
